...
CGPackage::CGPackage(CAESRijndael *p) : pszData(NULL), prijndael(p), lBufferSize(0), lReadPos(0), lWritePos(0)
{
	szReference[0] = 0x00;
}

CGPackage::CGPackage(CRawPackage *praw, CAESRijndael *p) : pszData(NULL), lBufferSize(0), lReadPos(0), lWritePos(0)
{
	prijndael = p;
	SetRawData(praw);
	szReference[0] = 0x00;
...
CGPackage::CGPackage(OL_UINT32 pkgSize, OL_UINT16 pkgId, CAESRijndael *p) : pszData(NULL), prijndael(p)
{
	szReference[0] = 0x00;
